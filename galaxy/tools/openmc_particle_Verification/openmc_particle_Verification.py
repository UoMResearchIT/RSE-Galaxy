...
def openmc_model(config_dict, geom_path, statepoint_path, threads):
    ...
    # Set triggers for tallies 
    for Tally in Tally_collect:
        Std_dev_trig = openmc.Trigger('rel_err', threshold=0.02) ## Threshold of the relative error, currently set to 2% error
        score_name = Tally.scores
        Std_dev_trig.scores = score_name
        
        Tally.triggers = [Std_dev_trig]
    ...
    model.settings.trigger_max_batches = model.settings.batches * 10 # Will run up to 10 times as long as the initial batch setting in the JSON
