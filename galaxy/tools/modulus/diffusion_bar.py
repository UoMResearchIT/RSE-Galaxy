...
# params for domain
tile = Tessellation.from_stl('sampleTiles.STL', airtight=True).scale(0.001)
tile_internal_sur = Tessellation.from_stl('sampleTiles_inner.STL', airtight=True).scale(0.001)
tile_outer_sur = Tessellation.from_stl('sampleTiles_outer.STL', airtight=True).scale(0.001)
# tile_side = Tessellation.from_stl('../../source_code/diffusion_1d/sampleTiles_side.STL', airtight=True)

# reactorwall = Tessellation.from_stl('../../source_code/diffusion_1d/samplePressureVessel.STL', airtight=True)
# reactorwall_inner_sur = Tessellation.from_stl('../../source_code/diffusion_1d/samplePressureVessel_inner.STL', airtight=True)
# reactorwall_outer_sur = Tessellation.from_stl('../../source_code/diffusion_1d/samplePressureVessel_outer.STL', airtight=True)
# reactorwall_side = Tessellation.from_stl('../../source_code/diffusion_1d/samplePressureVessel_side.STL', airtight=True)
...
def run(cfg: ModulusConfig) -> None:
    ...
    diff_u1 = Diffusion(T="u_1", D=D1, dim=3, time=False)
    diff_u2 = Diffusion(T="u_2", D=D2, dim=3, time=False)
    diff_in = DiffusionInterface("u_1", "u_2", D1, D2, dim=3, time=False)
    norm_diff_u1 = GradNormal("u_1", dim=3, time=False)
...
#     domain.add_constraint(interior_u2, "interior_u2")
    ...
    # Tile_data = np.loadtxt('cube.csv', delimiter=',', skiprows=1, usecols=(mapping.values()))
    # sampled_x = np.expand_dims(Tile_data[:, mapping["x"]].flatten(), axis=-1)
    # sampled_y = np.expand_dims(Tile_data[:, mapping["y"]].flatten(), axis=-1)
    # sampled_z = np.expand_dims(Tile_data[:, mapping["z"]].flatten(), axis=-1)
    # temperature = np.expand_dims(Tile_data[:, mapping["Temperature"]].flatten(), axis=-1)
    # # Create a PointwiseValidator object
    # validator_Tile = PointwiseValidator(
    #     nodes=nodes,
    #     invar = {"x": sampled_x, "y": sampled_y, "z":sampled_z},
    #     true_outvar = {"u_1": temperature},
    # )
    
    # domain.add_validator(validator_Tile, name="Val_Tile")
